...
export default function ChatPage({ onBack, onAccount }: Props) {
  ...
  }, [activeChat?.messages.length, isSearching]);
    ...
    <div className="flex h-dvh overflow-hidden bg-ink-50">
      ...
      <aside
        className={`${sidebarOpen
          ? 'translate-x-0'
          : '-translate-x-full'
          } fixed inset-y-0 left-0 z-30 flex w-[290px] flex-col border-r border-ink-200 bg-white transition-transform md:relative md:translate-x-0`}
      >

        <div className="flex h-[73px] items-center justify-between border-b border-ink-100 px-5">
          <Logo size="sm" />
        ...
        <div className="p-4">
          <button
            ...
            className="flex w-full items-center justify-center gap-2 rounded-xl bg-brand-600 px-4 py-3 text-sm font-bold text-white shadow-lg shadow-brand-600/20 transition-all hover:bg-brand-700"
          >
            <Plus size={17} />
            Nova conversa
          </button>
        ...
        <div className="scrollbar-thin flex-1 overflow-y-auto px-3 pb-5">
          ...
          <p className="mb-3 px-3 text-[10px] font-bold uppercase tracking-widest text-ink-400">
            Histórico de buscas
          </p>
          ...
          {Object.keys(groupedChats).length === 0 && (
            <div className="px-3 py-8 text-center">
              <MessageSquare
                size={28}
                className="mx-auto text-ink-200"
              />
          ...
          {Object.entries(groupedChats).map(
            ([group, items]) => (
              <div
                ...
                className="mb-5"
              >
                <p className="mb-1.5 px-3 text-[11px] font-medium text-ink-400">
                  {group}
                </p>
                ...
                {items.map(chat => (
                  <div
                    key={chat.id}
                    className={`group mb-1 flex cursor-pointer items-center gap-2 rounded-xl px-3 py-3 transition-colors ${activeId === chat.id
      ...
      <main className="flex min-w-0 flex-1 flex-col">
        ...
        <header className="flex h-[73px] shrink-0 items-center justify-between border-b border-ink-200 bg-white px-4 sm:px-7">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={onBack}
              className="hidden items-center gap-2 rounded-lg p-2 text-sm font-medium text-ink-500 transition-colors hover:bg-ink-50 hover:text-ink-800 sm:flex"
            >
              <ArrowLeft size={17} />
              Início
            </button>
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="flex items-center gap-1 text-[10px] text-brand-600">
          ...
          <button
            onClick={onAccount}
            className="hidden items-center gap-2 rounded-lg px-3 py-2 text-sm font-medium text-ink-500 transition-colors hover:bg-ink-50 hover:text-ink-800 sm:flex"
          >
            <Settings size={16} />
            Conta
          </button>
        ...
        <section className="scrollbar-thin flex-1 overflow-y-auto px-4 py-8 sm:px-8">
          ...
          <div className="mx-auto max-w-3xl">
            ...
            {!activeChat || activeChat.messages.length === 0 ? (
              ...
              <div className="flex min-h-[60vh] flex-col items-center justify-center text-center">
                ...
                <div className="mb-5 grid h-16 w-16 place-items-center rounded-2xl bg-brand-100 text-brand-600 shadow-inner">
                  <Search size={29} />
                </div>
                ...
                <div className="mt-8 grid w-full max-w-xl gap-2 sm:grid-cols-2">
                  ...
                  {SUGGESTIONS.map(suggestion => (
                    ...
                    >
                      ...
                      <ArrowUp
                        size={14}
                        className="ml-auto rotate-45 text-ink-300"
                      />
                    </button>
                  ))}
              ...
              <div className="space-y-7">
                ...
                {activeChat.messages.map(message => (
                  ...
                  >
                    ...
                    <div
                      className={
                        message.role === 'user'
                          ? 'max-w-[82%] rounded-2xl rounded-tr-sm bg-ink-900 px-4 py-3 text-sm leading-relaxed text-white shadow-sm'
                          : 'min-w-0 max-w-[90%] flex-1'
                      }
                    >

                      {message.role === 'assistant' ? (
                        <>
                          <div className="rounded-2xl rounded-tl-sm border border-ink-200 bg-white px-4 py-3 text-sm leading-7 text-ink-700 shadow-sm">
                ...
                {isSearching && (
                  <div className="flex gap-3 animate-fade-in">
                    ...
                    <div className="flex items-center gap-1 rounded-2xl rounded-tl-sm border border-ink-200 bg-white px-5 py-4">

                      <span className="h-2 w-2 animate-bounce-dot rounded-full bg-brand-500 [animation-delay:-0.3s]" />
                      <span className="h-2 w-2 animate-bounce-dot rounded-full bg-brand-500 [animation-delay:-0.15s]" />
                      <span className="h-2 w-2 animate-bounce-dot rounded-full bg-brand-500" />
