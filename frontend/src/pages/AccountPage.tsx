...
export default function AccountPage({ onBack, onSignOut }: Props) {
  ...
  return <div className="min-h-screen bg-ink-50">
    <header className="border-b border-ink-200 bg-white"><div className="mx-auto flex max-w-3xl items-center justify-between px-5 py-4 sm:px-8"><Logo size="sm" /><button onClick={onBack} className="inline-flex items-center gap-2 rounded-lg px-3 py-2 text-sm font-medium text-ink-500 hover:bg-ink-50 hover:text-ink-800"><ArrowLeft size={17} /> Voltar ao chat</button></div></header>
    <main className="mx-auto max-w-3xl px-5 py-10 sm:px-8"><h1 className="font-display text-2xl font-bold tracking-tight text-ink-950">Minha conta</h1><p className="mt-1.5 text-sm text-ink-500">Gerencie suas informações pessoais.</p>
      <div className="mt-8 rounded-2xl border border-ink-200 bg-white p-6 sm:p-8"><div className="flex items-center gap-4"><div className="grid h-16 w-16 place-items-center rounded-full bg-brand-600 text-lg font-bold text-white">{initials}</div><div className="min-w-0"><p className="truncate font-display text-lg font-bold text-ink-900">{profile?.full_name || 'Usuário'}</p><p className="truncate text-sm text-ink-500">{user?.email || 'E-mail de demonstração'}</p></div></div><div className="my-7 h-px bg-ink-100" />
        <div className="space-y-5"><Field label="Nome completo" icon={<User size={16} />}><input type="text" value={fullName} onChange={(event) => setFullName(event.target.value)} placeholder="Seu nome" className="w-full bg-transparent text-sm text-ink-900 outline-none placeholder:text-ink-400" /></Field><Field label="Telefone (opcional)" icon={<Phone size={16} />}><input type="tel" value={phone} onChange={(event) => setPhone(event.target.value)} placeholder="(11) 99999-9999" className="w-full bg-transparent text-sm text-ink-900 outline-none placeholder:text-ink-400" /></Field><div><span className="mb-1.5 block text-xs font-semibold text-ink-600">E-mail</span><div className="rounded-xl border border-ink-100 bg-ink-50/60 px-3.5 py-3 text-sm text-ink-500">{user?.email || 'E-mail de demonstração'}</div><p className="mt-1.5 text-xs text-ink-400">O e-mail será conectado ao backend posteriormente.</p></div>{error && <p className="rounded-lg bg-error-500/10 px-3 py-2 text-xs font-medium text-error-600">{error}</p>}<div className="flex items-center gap-3 pt-1"><button onClick={() => void handleSave()} disabled={saving} className="inline-flex items-center gap-2 rounded-xl bg-brand-600 px-5 py-3 text-sm font-bold text-white shadow-lg shadow-brand-600/20 hover:bg-brand-700 disabled:opacity-60">{saving ? <Loader2 size={16} className="animate-spin" /> : <Check size={16} />} Salvar alterações</button>{saved && <span className="animate-fade-in text-sm font-medium text-brand-600">Alterações salvas!</span>}</div></div>
      </div>
      <div className="mt-6 rounded-2xl border border-ink-200 bg-white p-6 sm:p-8"><h2 className="font-display text-lg font-bold text-ink-900">Sessão de demonstração</h2><p className="mt-1.5 text-sm text-ink-500">Os dados ficam somente neste navegador até o backend ser conectado.</p><button onClick={() => void handleSignOut()} className="mt-4 inline-flex items-center gap-2 rounded-xl border border-error-500/30 bg-error-500/5 px-5 py-3 text-sm font-semibold text-error-600 hover:bg-error-500/10"><LogOut size={16} /> Sair</button></div>
    </main>
  </div>;
}
