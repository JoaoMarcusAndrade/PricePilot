...
export default function SignupForm({ onGoogle, onToggle }: Props) {
  ...
  return (
    <div className="animate-fade-in">
      <h1 className="font-display text-2xl font-bold tracking-tight text-ink-950">Crie sua conta</h1>
      <p className="mt-2 text-sm text-ink-500">Grátis. Comece a comparar preços em segundos.</p>

      <button
        onClick={onGoogle}
        className="mt-7 flex w-full items-center justify-center gap-3 rounded-xl border border-ink-200 bg-white px-4 py-3 text-sm font-semibold text-ink-700 transition-all hover:border-ink-300 hover:bg-ink-50"
      >
      ...
      <div className="my-6 flex items-center gap-3">
        ...
        <span className="text-xs font-medium text-ink-400">ou com e-mail</span>
        <div className="h-px flex-1 bg-ink-100" />
      </div>
      ...
      <form onSubmit={(e) => { e.preventDefault(); handleSubmit(); }} className="space-y-4">
        ...
        <button
          type="submit"
          disabled={loading}
          className="flex w-full items-center justify-center gap-2 rounded-xl bg-brand-600 px-4 py-3 text-sm font-bold text-white shadow-lg shadow-brand-600/20 transition-all hover:bg-brand-700 disabled:opacity-60"
        >
      ...
      <p className="mt-6 text-center text-sm text-ink-500">
        Já tem conta?{' '}
        <button onClick={onToggle} className="font-bold text-brand-600 hover:text-brand-700">
          Entrar
        </button>
      </p>
    </div>
